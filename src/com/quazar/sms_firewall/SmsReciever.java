				...
				if (!blocked) {
					...
					for (TopFilter ti:topFilters) {
						if (ti.getType() == TopType.PHONE_NAME
								...
								&& body.toLowerCase(Locale.getDefault()).contains(
										ti.getValue().toLowerCase(Locale.getDefault()))) {
					...
					if (fraudDetected) {
						String text =
								String.format(context.getResources().getString(R.string.fraud_warn), DictionaryUtils
										.getInstance().getContactsName(fraudNumber));
						NotificationUtil.warning(context, R.drawable.warning, R.string.warning, text);
					}
